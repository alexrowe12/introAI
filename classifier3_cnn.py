"""
Classifier 3: Convolutional Neural Network (CNN) for X vs O Classification

Trained on the same dataset as Classifier 2, but uses spatial convolutions
instead of flattened linear layers. Operates on 32x32 input (vs MLP's 16x16)
to give conv layers more spatial signal to work with.

Architecture:
- Input: 32x32 binary image
- Conv2d(1, 16, 3) + ReLU + MaxPool → 16x16x16
- Conv2d(16, 32, 3) + ReLU + MaxPool → 32x8x8
- Flatten → 2048
- FC(2048, 64) + ReLU
- FC(64, 1) + Sigmoid
"""
...
class SimpleCNN(nn.Module):
    ...
    Architecture:
    - Input: flattened image (grid_size * grid_size), reshaped internally to 2D
    - Conv block 1: 16 filters, 3x3 kernel, ReLU, MaxPool2d(2)
    - Conv block 2: 32 filters, 3x3 kernel, ReLU, MaxPool2d(2)
    - FC: 2048 → 64 → 1 (Sigmoid)
    """
    ...
    def __init__(self, grid_size=32):
        ...
        # After two MaxPool2d(2) on a grid_size x grid_size input:
        # spatial dims = grid_size // 4
        pool_size = grid_size // 4
        fc_input = 32 * pool_size * pool_size
    ...
    def forward(self, x):
        # x is (batch, grid_size*grid_size) — reshape to (batch, 1, H, W)
        batch = x.size(0)
        x = x.view(batch, 1, self.grid_size, self.grid_size)
...
def preprocess_image(image_path, grid_size=32, augment=False):
    """
    Preprocess image:
...
class XODataset(Dataset):
    ...
    def __init__(self, image_dirs, labels_dict, grid_size=32, augment=False, augment_factor=10):
        self.grid_size = grid_size
...
def train_model(model, train_loader, epochs=100, lr=0.001, verbose=True, device=None):
    """Train the CNN model."""
    if device is None:
...
def evaluate_model(model, image_dirs, labels_dict, grid_size=32, verbose=True, device=None):
...
def load_model(path):
    ...
    model = SimpleCNN(grid_size=checkpoint['grid_size'])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train and test CNN classifier for X vs O")
    parser.add_argument("-d", "--dir", type=str, nargs="+", default=["trainingData"],
                        help="Directories containing images (default: trainingData)")
    parser.add_argument("--train", action="store_true",
                        help="Train a new model")
    parser.add_argument("--epochs", type=int, default=50,
                        help="Number of training epochs (default: 50)")
    parser.add_argument("--model", type=str, default="cnn_model.pth",
                        help="Path to save/load model (default: cnn_model.pth)")
    ...
    if args.train:
        ...
        dataset = XODataset(image_dirs, LABELS, grid_size=32, augment=True, augment_factor=10)
        train_loader = DataLoader(dataset, batch_size=16, shuffle=True)

        model = SimpleCNN(grid_size=32)
    ...
    print("EVALUATION")
    print("-" * 40)
    results = evaluate_model(model, image_dirs, LABELS, grid_size=32, verbose=True, device=device)

    print()
    print("=" * 60)
    print("USAGE:")
    print("  python classifier3_cnn.py                    # Evaluate with saved model")
    print("  python classifier3_cnn.py --train            # Train new model")
    print("  python classifier3_cnn.py --train --gpu      # Train using GPU (Apple MPS)")
    print("  python classifier3_cnn.py --train --epochs 200")
    print("  python classifier3_cnn.py -d testSet         # Evaluate on test set")
    print("=" * 60)
